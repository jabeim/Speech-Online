...
%     analysisFreqs = generate_cfs(analysisLo, analysisHi, nElectrodes);      % electrodes' centre frequencies
    ...
    % allow for comparison between 'standard' tone vocoder and spiral
    if carrierDensity == 0 
        % Density 0 means that output tones match analysis bands
        carrierFreqs = analysisFreqs;
    else
        % Density greater than zero specifies how many tones per ERB within the range [carrierLo carrierHi]
...
%             processedRange = 20*log10(max(compressedEnvelope(:,j).^2)/min(compressedEnvelope(:,j).^2))
            compressedEnvelope(envelope(:,j) >= envMax(j),j) = 0; % zero points above envMax to reduce signal noise
        else
